...
class default_model extends model
{
    ...
    public function getFromParams($params, $show = 0)
    {
        $select = array();
        if($params['select'])
        {
                    ...
                    if(is_array($v['value'])) {
                        ...
                        if($v['sign'] != 'in') {
                            foreach ($v['value'] as $value) {
                                $parts[] = $k . ' ' . ($value['sign'] ? $value['sign'] . ($value['value'] == 'NULL' || $value['noquotes'] ? ' ' . $value['value'] : ' "' . $value['value'] . '"') : '"' . $v . '"');
                            }
                        }
                        if($v['interfix'] == 'or') {
        ...
        $group = $params['group'];
        $having = $params['having'];
        $limits = $params['limits'] ? $params['limits'] : '';
        $stm = $this->pdo->prepare('
        SELECT
        ' . ($select ? implode(',',$select) : '*') . '
        FROM
